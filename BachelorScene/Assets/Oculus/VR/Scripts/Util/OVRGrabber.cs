...
public class OVRGrabber : MonoBehaviour
{
    ...
    public Dictionary<OVRGrabbable, int> grabbingCandidates
    {
        get { return m_grabCandidates; }
    }
    ...
    void OnUpdatedAnchors()
    {
		...
		m_prevFlex = OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger, Controller);
    ...
    protected virtual void GrabBegin()
    {
		...
		foreach (OVRGrabbable grabbable in m_grabCandidates.Keys)
        {
            ...
            if (!canGrab || !grabbable.enabled)
            {
                continue;
            }

            float radiusSquared = Mathf.Pow(m_collider.radius, 2);
            ...
            for (int j = 0; j < grabbable.grabPoints.Length; ++j)
            {
                ...
                if (grabbableMagSq < closestMagSq && grabbableMagSq <= radiusSquared)
                {
        ...
        if (closestGrabbable != null)
        {
            ...
            Transform grabbedTransform = m_grabbedObj.invdividualRigidBodies ? m_grabbedObj.grabbedTransform : m_grabbedObj.transform;
            ...
            {
                Vector3 relPos = grabbedTransform.position - transform.position;
            ...
            {
                Quaternion relOri = Quaternion.Inverse(transform.rotation) * grabbedTransform.rotation;
